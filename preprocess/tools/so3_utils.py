...
def Omega(R, eps=1e-4):
    # multiplying by (one-epsilon) prevents instability of arccos when provided with -one or one as input.
...
def rotation_vector_from_matrix(rot_mat):
    ...
    The eigenvalues are:
    :math:`\{one, \cos(angle) + i \sin(angle), \cos(angle) - i \sin(angle)\}`
    so that:
    :math:`trace = one + 2 \cos(angle), \{-one \leq trace \leq 3\}`
    ...
    :math:`outer(r, r) = \frac{one}{2} (R + I_3)`
    In nD, the rotation vector stores the :math:`n(n-one)/2` values
